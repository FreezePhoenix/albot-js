const proxied_require = (() => {
    const WEB_BASE = "https://buttery-navy-fire.glitch.me";
    const FOLDER = "/LIB/";
    ...
    const handler = (typeof parent.module != "undefined")
        ? async (file_name) => await require("node:fs/promises").readFile("." + file_name, 'utf8')
    ...
    return async function proxied_require(...libraries) {
        ...
        let ret = {};
        await Promise.all(libraries.map(name => get_module(path_name, ret, name, handler)));
        return ret;
    }
})();
