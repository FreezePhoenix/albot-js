...
let monster_targets = ['fireroamer', 'ent'],
	state = 'farm',
...
let ORBIT_SPEED = 40;
let ORBIT_DISTANCE = 25;
const ORBIT_TARGET = { real_x: 250, x: 250, real_y: -812, y: -812, map: "desertland", in: "desertland" };
...
if (character.name == 'Geoffriel') {
	if (AOE) {
		setInterval(() => {
			let can_zap = can_use('zapperzap');
			let physical_targeting = 0;
			if (can_zap) {
				for (let X in parent.entities) {
					let entity = parent.entities[X];
					if (
						entity.type == 'monster' &&
						entity.mtype == 'fireroamer'
					) {
			...
			let can_absorb = can_use('absorb');
			let targeting_rael = 0,
				targeting_raelina = 0;
			if (can_absorb) {
				for (let X in parent.entities) {
					let entity = parent.entities[X];
					if (
						...
						if (entity.target == 'Rael') {
							targeting_rael++;
						} else if (entity.target == 'Raelina') {
							targeting_raelina++;
						}
					}
				}
				if (targeting_rael > 2 || targeting_raelina > 2) {
					if (targeting_rael > targeting_raelina) {
						absorb('Rael');
					} else {
						absorb('Raelina');
					}
		...
		setInterval(() => {
			...
			for (let X in parent.entities) {
				let entity = parent.entities[X];
				if (entity.type == 'monster' && entity.mtype == 'fireroamer') {
			...
			let can_zap = can_use('zapperzap') && !targets;
			if (can_zap) {
				for (let X in parent.entities) {
					let entity = parent.entities[X];
					if (
	...
	setInterval(() => {
		let ALTERNATE = true;
		let ent_targeting_self = -1,
			ents = 0,
			roamers_targeting_self = 0;
		for (let id in parent.entities) {
			let entity = parent.entities[id];
			if (entity.type == 'monster') {
				if (entity.target == character.name) {
					if (entity.mtype == 'ent') {
						ents_targeting_self = id;
						ents++;
					} else if (entity.mtype == 'fireroamer') {
						roamers_targeting_self++;
					}
				} else {
					if (entity.mtype == 'ent') {
						ents++;
		...
		if (ent_targeting_self != -1) {
			if (!ALTERNATE && can_use('hardshell')) {
				hardshell();
			}
				...
				if (entity.type == 'monster') {
					if (
						...
						(ALTERNATE ||
							...
							entity.target == 'Geoffriel')
					) {
						if (distance(character, entity) < 200) {
							if (ALTERNATE) {
								if (
									can_use('hardshell') &&
									parent.entities[entity.target]?.s
										.hardshell == null
								) {
									hardshell();
									taunt(entity.id);
...
if (AOE) {
	if (character.name == 'Rael') {
		...
		MAINHAND_FILTER = ItemFilter.ofName('vhammer')
			.level('8', '==')
			.property('firehazard')
			.build();
		// OFFHAND_FILTER = ItemFilter.ofName("vhammer").level("8", "==").property(false).build();
...
if (character.name == 'Rael' || character.name == 'Raelina') {
	...
	setInterval(() => {
		if (character.s.sugarrush) {
			// ensure_equipped(DGLOVE_FILTER, "gloves");
			ensure_equipped(PCHEST_FILTER, 'chest');
		} else {
...
if (character.name == 'Geoffriel') {
	const GOLD_RING = ItemFilter.ofName('goldring').build();
	const GOLD_GLOVE = ItemFilter.ofName('handofmidas')
		.level('7', '==')
		.build();

	const DPS_RING1 = ItemFilter.ofName('zapper').build();
	const DPS_RING2 = ItemFilter.ofName('cring').level('4', '==').build();
	const DPS_GLOVE = ItemFilter.ofName('gloves').level('13', '==').build();
	...
	setInterval(() => {
		...
		if (chests > 10) {
			...
			for (let chest_id in parent.chests) {
				...
				if (!looted) {
					let index_of_booster =
						character.items.findIndex(BOOSTER_FILTER);
...
async function farm(location = ORBIT_TARGET) {
	...
	if (attack_target != null) {
			...
			if (can_use('attack', NOW)) {
				...
				for (let x in parent.entities) {
					...
					if (
						entity.type == 'monster' &&
						entity.mtype == 'fireroamer' &&
						(entity.target == null || entity.target == character.name)
					) {
						total++;
					}
				}
				if (
					...
					unequip('offhand');
					ensure_equipped(AXE_FILTER, 'mainhand');
					cleave();
					try {
		...
		if (curEvent == null && distance_to_point(location.x, location.y) > 100) {
			move_to(location)
		}
	}
}
