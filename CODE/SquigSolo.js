...
let targets = {
	Raphiel3: ["squigtoad", "frog", "squig"],
	Firenus: ["squigtoad", "frog", "squig"],
	Boismon: ["squigtoad", "frog", "squig"],
	Daedulaus: ["squigtoad", "frog", "squig"],
	"AriaHarper": ["squigtoad", "frog", "squig"],
  Geoffriel: ["squigtoad", "frog", "squig"]
...
let monster_targets = targets[character.name],
	state = "farm",
	group = [character.name, "Geoffriel", "Daedulaus", "AriaHarper"], // The group our character operates in. This generally forms a chain.
	to_party = ["Boismon", "Firenus", "Daedulaus", "Geoffrey", "AriaHarper", "Geoffriel", "Raphiel3"],
...
let to_sell = new Set(["ringsj", "hpamulet", "intamulet", "dexamulet", "stramulet"]);
let to_send = new Set(["egg0", "egg1", "egg2", "egg3", "egg4", "egg5", "egg6", "egg7", "egg8", "mcape", "firestaff", "fireblade", "armorbox", "weaponbox", "gem1", "cupid", "hpbelt", "essenceoffrost", "intring", "dexring", "strring", "candypop", "intbelt", "dexbelt", "strbelt", "redenvelopev2", "candy0v3", "candy1v3", "mpot0", "hpot0", "", "lostearring", "mistletoe", "candycane", "ornament", "intearring", "dexearring", "vitearring", "strearring", "vitscroll"]);
// /*
setInterval(() => {
		...
		for (let i = 0, len = items.length; i < len; i++) {
			...
			if (item != null) {
				if (to_sell.has(item.name) && sold < max_send && (item.level == 0 || item.level == null)) {
					socket.emit("destroy",{num:i,q:1,statue:true});
					sold++
				}
...
function farm() {
	let possible_follows = (follow) ? to_party.slice(chain.indexOf(character.name) + 1).map(get_player) : [];
	...
	switch (character.ctype) {
		...
		case "mage":
			...
			}), get_player("Boismon")].filter((char) => {
				return char != null
			}).sort((a, b) => {
	...
	if (character.ctype != "merchant") {
		...
		if (attack_target != null) {
            ...
            if(character.ctype == "rogue" && can_use('quickstab') && distance_to_point(target.real_x, target.real_y) < character.range) {
									use_skill("quickstab", target)
								}
							if(character.ctype == "rogue" && can_use('mentalburst') && distance_to_point(target.real_x, target.real_y) < character.range + 32 + 10 + character.xrange) {
						...
						if (character.ctype == "priest" && attack_target.mtype == "ghost" && !healed_ghosts.has(attack_target.id) && true) {
							...
							if (can_attack(attack_target)) {
								...
								if (item != null && item.name == "wbook0" && ["priest", "mage"].includes(character.ctype)) {
									if (character.slots.offhand.name == "lantern" && false) {
										equip(41)
										allow_swap = false;
										setTimeout(() => {
											allow_swap = true;
                ...
                if(character.ctype == "ranger" && can_use("huntersmark")) {
								...
								if (false && character.ctype == "mage" && can_use("cburst") && true) {

									parent.socket.emit("skill", {
										name: "cburst",
										targets: find_viable_targets().slice(0, 10).filter((entity) => {
									...
									if ((targets.every((entity) => {
										...
										ThreeShotTargets = find_viable_targets().slice(0, 3).map((entity) => entity.id);
										   ...
										   (targets = find_viable_targets().slice(0, 3).filter((entity) => {
									return parent.distance(entity, character) < character.range + 10
								})).map(({
									id
								}) => id).length >= 2 && character.level >= 60 && (character.mp > 300) && true) {
									if ((targets.every((entity) => {
										return entity.hp > character.attack * 0.7 * 1.1;
									}) && character.hp != character.max_hp) || (targets.every((entity) => {
										return entity.hp > character.attack * 0.7 * 1.1 * 2;
										...
										parent.socket.emit("skill", {
											name: "3shot",
											ids: find_viable_targets().slice(0, 3).map((entity) => entity.id)
										})
										ThreeShotTargets = find_viable_targets().slice(0, 3).map((entity) => entity.id);
										if(index != -1) {
											equip(index)
										...
										parent.socket.emit("skill", {
											name: "3shot",
											ids: find_viable_targets().slice(0, 3).map(({
												id
											}) => id)
										});
										ThreeShotTargets = find_viable_targets().slice(0, 3).map((entity) => entity.id);
								...
								setTimeout(() => {
									let item = character.items[41]
									if (item != null && item.name == "lantern" && ["priest", "mage"].includes(character.ctype) && true) {
		...
		if((distance_to_point(-295,558) > 5 || character.map != "level1") && !smart.moving && monster_targets[0] == "prat") {
			smart_move({
				x: -295,
				y: 587,
				map: "level1"
