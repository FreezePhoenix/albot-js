...
function orbit_entity(entity, distance) {
	...
	var position = pointOnAngle(entity, point + (character.speed / (2 * Math.PI * distance)) % 360, distance)
...
let monster_targets = targets[character.name],
	 ...
	 to_party = ["Rael", "Geoffriel", "Boismon", "Geoffriel", "AriaHarper", "Geoffriel", "Raphiel"],
	 party_leader = "Rael",
...
let allow_swap = 1;
const orbit_target = {real_x:20, real_y: -1109};
...
function farm() {
	...
	if (character.ctype != "merchant") {
		...
		let possible_targets = follow ? get_target_of(follow_target) : (find_viable_targets() || []);
		if(character.name == "Firenus") {
			possible_targets = possible_targets.filter((target) => {
				return parent.distance(target, character) < character.range + 10
			}).splice(0, 2).reverse();
		}
		let attack_target = possible_targets[0];
		//Attack or move to target
		...
		if (attack_target != null) {
			...
			let viable_targets = find_viable_targets();
			if (!group.includes(attack_target.id)) {
				if (parent.distance(attack_target, character) < character.range + 10) {
					if (can_attack(attack_target)) {
						...
						if (character.ctype == "priest" && attack_target.mtype == "ghost" && !healed_ghosts.has(attack_target.id) && true) {
							...
							if (can_attack(attack_target)) {
								...
								if (character.ctype == "mage" && can_use("cburst") && true) {
									parent.socket.emit("skill", {
										...
										targets: find_viable_targets().slice(0, 2).filter((entity) => {
											...
											id}) => [id, 1])
											  ...
											  can_use("attack") &&
											  (targets = viable_targets.slice(0, 5).filter((entity) => {
									return parent.distance(entity, character) < character.range + 10
								}).map(({
									id
								}) => id)).length >= 4 && character.level >= 75 && (character.mp > 420) && true) {
										parent.socket.emit("skill", {
											name: "5shot",
											ids: targets
										});
									...
									return parent.distance(entity, character) < character.range + 10
								})).length >= 2 && character.level >= 60 && true) {
									let is_5shot;
									let targets = viable_targets.slice(0, 3)
									if(targets.filter((entity) => entity.target != character.name).length == 2) {
										...
										targets.push(...((targeter2.GetPriorityTarget(2) || []).filter((entity) => entity.target == character.name || entity.target == undefined) || []));
										let new_targets = targets.slice(0, 5).map(({
												id
											}) => id);
										parent.socket.emit("skill", {
											name: "5shot",
					...
					if (can_move_to(target.x, target.y) && target.mtype != "prat" && ["rogue", "warrior"].includes(character.ctype)) {
						move_to_target(attack_target)
					} else if (!smart.moving && target.mtype != "prat" && character.ctype == "rogue") {
						smart_move({
							x: target.real_x,
							y: target.real_y
						})
