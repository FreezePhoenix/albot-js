...
let monster_targets = targets[character_name],
  state = "farm",
  group = ["Geoffriel", character.name], // The group our character operates in. This generally forms a chain.
  to_party = ["Rael", "Daedulaus", "Raelina", "Geoffriel", "AriaHarper"],
  party_leader = "Rael",
...
let wabbit = null;
let curEvent = null;
if (character.name == "Rael" || character.name == "Geoffriel") {
  setInterval(function () {
    curEvent = events.find((item) => {
      return "easterluck" in character.s ? false : item.name === "wabbit";
    });
    let data =
      typeof parent.socket.server_data == "undefined"
        ? parent.S
        : parent.socket.server_data;
...
if (character.name == "Geoffriel") {
  setInterval(() => {
    ...
    if (character.name == "Geoffriel") {
      Object.keys(parent.chests).forEach((id) => {
        if (!looted_chests.has(id)) {
          ...
          if (!switched) {
            ensure_equipped("goldring", "ring2");
            ensure_equipped("handofmidas", "gloves");
    ...
    if (switched) {
      if (switched) {
        setTimeout(() => {
          let index = character.items.findIndex((item) => {
            return item && item.name == "goldbooster";
          });
          ensure_equipped("wgloves", "gloves");
          ensure_equipped("ringofluck", "ring2");
          shift(index, "luckbooster");
...
var kiting_range = (2 * character.range) / 4;
...
if (character.name == "Geoffriel") {
  setInterval(() => {
    if (
      ...
      if (parent.entities[character.target]) {
        current_target = character.target;
        kiting_clockwise = determine_clockwise(
          kiting_origin,
          parent.entities[character.target],
          kiting_range
        );
        let movePoint = get_kite_point(
          kiting_origin,
          parent.entities[character.target],
          kiting_range,
          kiting_clockwise
    ...
    case "priest":
      ...
      }
      {
        let possible_targets = follow
          ...
          : find_viable_targets() || [];
        possible_targets = possible_targets.filter((ent) => {
          return !to_party.includes(ent.target);
        });
        let attack_target = (target = Array.isArray(possible_targets)
          ? possible_targets[0]
          : possible_targets);
        if (attack_target) {
          if (attack_target.target != character.name) {
            if (can_use("zapperzap")) {
              parent.socket.emit("skill", {
                name: "zapperzap",
                id: attack_target.id,
              });
              // Not extactly correct
              healed = true;
            }
          }
        }
      ...
      if (!group.includes(attack_target.id)) {
        if (parent.distance(attack_target, character) < character.range + 10) {
          if (can_use("attack")) {
            ...
            if (character.ctype == "priest") {
                parent.send_target_logic(attack_target, null);
          ...
          if (can_move_to(target.x, target.y) && true) {
            move_to_target(attack_target);
          } else if (!smart.moving && true) {
            smart_move({
              x: target.real_x,
              y: target.real_y,
            });
