...
let monster_targets = targets[character_name],
	state = "farm",
	group = ["Rael", "Geoffriel", "Raelina"], // The group our character operates in. This generally forms a chain.
	to_party = [
		"Rael",
		"Daedulaus",
		"Raelina",
	...
	follow_distance = 50, // How far should we be from the person we are following?
	merchant = "AriaHarper",
	priest = "Geoffriel",
	warrior_tank = true,
	need_priest = true,
...
setInterval(function() {
  ...
  if(parent.socket.server_data.wabbit && parent.socket.server_data.wabbit.live) {
    if(!character.s.easterluck) {
      if(events[0] == undefined) {
        events.push({x: parent.socket.server_data.wabbit.x, y:parent.socket.server_data.wabbit.y, name: "wabbit", map: parent.socket.server_data.wabbit.map});
      }
    }
  }
...
function farm() {
	...
	: [];
	let follow_target = get_first_non_null(possible_follows);
	if (follow_target && character.name != "Raelina") {
	...
	if (character.ctype != "merchant") {
		...
		if (attack_target != null && !ranger_healed) {
			...
			if (!smart.moving && !follow_target && !character.rip && !tree && true) {
				if (monster_targets[0] === "prat" || false) {
					smart_move("ghost", () => {
						smart_move({
							x: -295,
							y: 587,
		...
		if (
			monster_targets[0] == "prat" &&
			(distance_to_point(-295, 558) > 5 || character.map != "level1") &&
			!smart.moving
		) {
			smart_move(
				{
	...
	setInterval(() => {
		if (character.name == "Rael") {
			if (
				...
				parent.entities[character.target].target === character.name
			) {
				if (
...
switch (character.name) {
	case "Rael":
		guard_post({ x: -715, y: -2175, map: "desertland" });
		break;
	case "Raelina":
		guard_post({ x: -700, y: -2175, map: "desertland" });
		break;
}
...
function loadPvPTrackerLog() {
	curLog = null; // JSON.parse(localStorage.getItem("PvP_Log:" + character.name));
