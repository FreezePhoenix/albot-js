...
let monster_targets = targets[character_name],
	state = "farm",
	group = ["Geoffriel", "Raelina", "Rael"], // The group our character operates in. This generally forms a chain.
	to_party = [
		"Rael",
		"Daedulaus",
		"Raelina",
...
setInterval(() => {
	...
	if (merchant_near()) {
		...
		if (
			character.name == "Geoffriel" && num_items("elixirluck") <= 1 &&
...
if (character.ctype == "warrior") {
	setInterval(() => {
		if (!get_player(priest) || get_player(priest).rip && need_priest) {
			let targeted = false;
					...
					if (
						...
						if (this.TargetingPriority[entity.mtype] != null) {
							if (
								...
								if(entity.s.burned && entity.s.burned.intensity / 5 * (1 / 0.21) * (new Date() - new Date(entity.s.burned.last)) /100 > entity.hp) {
									continue;
								}
								let targetArgs = {
